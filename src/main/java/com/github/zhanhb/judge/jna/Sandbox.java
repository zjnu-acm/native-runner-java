...
import static com.github.zhanhb.judge.jna.Kernel32Ex.*;
import com.github.zhanhb.judge.jna.Kernel32Ex.JOBOBJECT_BASIC_LIMIT_INFORMATION;
import com.github.zhanhb.judge.jna.Kernel32Ex.JOBOBJECT_BASIC_UI_RESTRICTIONS;
...
public class Sandbox implements Closeable {
    ...
    public Sandbox() {
        hJob = Kernel32Ex.INSTANCE.CreateJobObject(null, null);
        ...
        if (!Kernel32Ex.INSTANCE.SetInformationJobObject(hJob, JobObjectBasicLimitInformation, jobli, jobli.size())) {
        ...
        if (!Kernel32Ex.INSTANCE.SetInformationJobObject(hJob, JobObjectBasicUIRestrictions, jobuir,
                jobuir.size())) {
            throw new Win32Exception(Kernel32.INSTANCE.GetLastError());
        }
    ...
    public void beforeProcessStart(HANDLE hProcess) {
        if (!Kernel32Ex.INSTANCE.AssignProcessToJobObject(hJob, hProcess)) {
            throw new Win32Exception(Kernel32.INSTANCE.GetLastError());
        }
    }
